...
# ─── Pagina-configuratie (moet als eerste) ────────────────────────────────────
st.set_page_config(
...
def display_task_input_page():
    ...
    st.markdown("Voer in hoeveel uur uw team *gemiddeld per week* besteedt. Zet 0 indien niet van toepassing.")
        ...
        with st.expander(f"**{category}**", expanded=True):
            for task_id, label in tasks:
                st.number_input(
                    ...
                    help=f"Uren per week aan: {label}"
                )
...
def display_cost_input_page():
    ...
    st.selectbox(
        ...
        ("Innovatie & productontwikkeling", "Klantrelaties", "Strategische planning", "Training & ontwikkeling"),
...
def display_results_page():
    ...
    with st.expander("Details & Methodologie"):
        st.dataframe(df)
        st.markdown("Gebaseerd op McKinsey (2023), Stanford (2024) en Wereldbank.")
